...
export const localizePath = (pageContext) => {
  console.log(
    "localizePath",
    pageContext.localizations.data,
    pageContext.locale
  );
  ...
  let localeFound = localizations.data.find(
    (a) => a.attributes.locale === locale
  ); // it will look in the localizations array of the 'primera-pagina' page
  console.log("localeFound", localeFound);
  if (localeFound)
    return formatSlug(localeFound.attributes.slug, locale, defaultLocale);
